...
BankBalanceManager _bankBalanceManager;
	...
	public Client checkExistence(String userName, int password, Connection connection) 
	{
				...
				while( (rs.next()) && (!found))
						{
							...
							if (userName.equals(userNameDB) && password == passwordDB) //if the password was found
						    	{
	...
	public void addCustomer(Client client, Account account, Connection connection, AccountManager accountManager)
	{
		...
		try {
			...
			if (checkUserName(client.get_clientUserName(), connection)) //check if there's already a client with the same password
			{
				String insertValues = 
				...
				System.out.println ("this user name already exist. Please change it.");
				}catch (SQLException sqle)
				{
	...
	private boolean checkUserName (String userName, Connection connection) throws SQLException //check if the password already exists
	{
		if (exist ("customers", "username", userName, connection))
			return false;
	...
	private boolean exist (String table, String field, String id, Connection connection) throws SQLException
	{
		Statement statement = getStatement(connection);
